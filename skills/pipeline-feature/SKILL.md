...
1. Generate a branch name from the task description: `feat/<short-slug>` (e.g., `feat/add-user-auth`)
2. **Initialize state file (Step 0)**: Follow the **Step 0: Initialize State File** procedure from pipeline-shared. This is the FIRST action — creates `.pipeline-state/<branch-name>.json` from `templates/feature-state.json`. If it already exists, this is a resume.
